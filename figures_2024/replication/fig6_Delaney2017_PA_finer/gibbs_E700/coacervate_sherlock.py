...
A_mon = p.Monomer("A", 1.0)
B_mon = p.Monomer("B", -1.0)
S_mon = p.Monomer("S", 0)
...
gibbs_time = 0.03
volume_time = 0.04
gibbs_sys = g_e.GibbsEnsemble(
    ps, integrator_1, gibbs_time, volume_time, spec_dict_2=spec_dict_2, grid_2=grid_2
)
gibbs_sys.part_2.get_densities()
# gibbs_sys.bind_species(A_poly, B_poly)
gibbs_sys.burn(500)
traj = []
vol_traj = []
for i in range(1000):
    ...
    traj.append(
        cp.concatenate(
            (
                gibbs_sys.C_1,
                gibbs_sys.C_2,
                gibbs_sys.sampled_pot_1,
                gibbs_sys.sampled_pot_2,
            )
        )
    )
    vol_traj.append(
        (
            gibbs_sys.part_1.grid.V,
            gibbs_sys.part_2.grid.V,
            gibbs_sys.sampled_pressure_1,
            gibbs_sys.sampled_pressure_2,
        )
    )
    if i % 1 == 0:

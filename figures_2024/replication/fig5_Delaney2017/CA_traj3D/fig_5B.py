...
for file in right_files:
    ...
    points.append(
        [
            A_amt,
            np.average(traj[indices, 1]),
            np.var(traj[indices, 1]) ** (0.5),
            -np.average(traj[indices, 2]),
            -np.var(traj[indices, 2]) ** (0.5),
            np.average(traj[indices, 3]),
            np.var(traj[indices, 3]) ** (0.5),
            len(indices),
        ]
    )
...
ax_mu.scatter(
    ref[:, 0], ref[:, 1], color="black", label=r"$\textrm{Delaney et al.}$", marker="_"
)
ax_mu.scatter(
    points[:, 0],
    points[:, 5],
    color="red",
    marker="_",
    label=r"$\textrm{Numeric $\mu$}$",
)
ax_mu.errorbar(
    points[:, 0],
    points[:, 5],
    yerr=points[:, 6] / points[:, -1] ** (1 / 2) * 1.69,
    ls="none",
    color="red",
)

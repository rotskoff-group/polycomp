...
for i in range(0, dens_traj.shape[0]):
    ...
    im[0][0] = axes[0, 0].imshow(B_traj[i].get(), cmap=yellow_cmap, vmin=0, alpha=0.6)
    im[0][0] = axes[0, 0].imshow(A_traj[i].get(), cmap=red_cmap, vmin=0, alpha=0.6)
    im[0][0] = axes[0, 0].imshow(L_traj[i].get(), cmap=green_cmap, vmin=0, alpha=0.6)
    im[2][1] = axes[2, 1].imshow(S_traj[i].get(), cmap=blue_cmap, vmin=0, alpha=0.6)
    im[1][0] = axes[1, 0].imshow(B_traj[i].get(), cmap=yellow_cmap, vmin=0, alpha=0.6)
    im[1][1] = axes[1, 1].imshow(A_traj[i].get(), cmap=red_cmap, vmin=0, alpha=0.6)
    im[2][0] = axes[2, 0].imshow(L_traj[i].get(), cmap=green_cmap, vmin=0, alpha=0.6)
...
plt.plot(
    where.get(), L_1D.get(), label=r"$\textrm{Lipid}$", color=colors[3], linewidth=3
)
plt.plot(
    where.get(), B_1D.get(), label=r"$\textrm{Cation}$", color=colors[2], linewidth=3
)
plt.plot(
    where.get(), A_1D.get(), label=r"$\textrm{mRNA}$", color=colors[1], linewidth=3
)

...
q_min = 2 * math.pi / (box_length[0] / 2)
q_max = 2 * math.pi / (box_length[0] / grid_spec[0])
...
fig.suptitle(
    "CART-Homopolymer Base Case \n(mRNA, Cat:Lip; 1,1:1) Salt=%.1f, B=%.1f"
    % (s_value, b_value),
    fontsize=10,
)
...
axes[0, 0].set_title("Max Densities")
...
red_cmap = LinearSegmentedColormap.from_list(
    "reds", (red_color + [0], red_color + [1]), N=256
)

blue_color = [68 / 256, 119 / 256, 170 / 256]
blue_cmap = LinearSegmentedColormap.from_list(
    "blues", (blue_color + [0], blue_color + [1]), N=256
)

cyan_color = [102 / 256, 204 / 256, 238 / 256]
cyan_cmap = LinearSegmentedColormap.from_list(
    "cyans", (cyan_color + [0], cyan_color + [1]), N=256
)

green_color = [34 / 256, 136 / 256, 51 / 256]
green_cmap = LinearSegmentedColormap.from_list(
    "greens", (green_color + [0], green_color + [1]), N=256
)

yellow_color = [204 / 256, 187 / 256, 68 / 256]
yellow_cmap = LinearSegmentedColormap.from_list(
    "yellows", (yellow_color + [0], yellow_color + [1]), N=256
)

grey_color = [187 / 256, 187 / 256, 187 / 256]
grey_cmap = LinearSegmentedColormap.from_list(
    "greys", (grey_color + [0], grey_color + [1]), N=256
)

for i in range(0, dens_traj.shape[0]):
    im[0][1] = axes[0, 1].imshow(T_traj[i].get(), cmap="Greys", vmin=0)
    im[2][1] = axes[2, 1].imshow(S_traj[i].get(), cmap="Blues", vmin=0)
    im[1][0] = axes[1, 0].imshow(B_traj[i].get(), cmap="Purples", vmin=0)
    im[1][1] = axes[1, 1].imshow(A_traj[i].get(), cmap="Reds", vmin=0)
    im[2][0] = axes[2, 0].imshow(L_traj[i].get(), cmap="Greens", vmin=0)
    ...
    im[0][0] = axes[0, 0].imshow(max_species_indices_np, cmap=cmap, origin="lower")
...
struct = plt.figure(figsize=(5.5, 4))
# plt.plot(where.get(), L_1D.get(), label=r'$\textrm{Lipid}$', color=colors[3])
plt.plot(where.get(), L_1D.get(), label=r"$\textrm{Lipid}$", color="forestgreen")
# plt.plot(where.get(), B_1D.get(), label=r'$\textrm{Cation}$', color=colors[2])
plt.plot(where.get(), B_1D.get(), label=r"$\textrm{Cation}$", color="firebrick")
# plt.plot(where.get(), A_1D.get(), label=r'$\textrm{mRNA}$', color=colors[1])
plt.plot(where.get(), A_1D.get(), label=r"$\textrm{mRNA}$", color="mediumblue")
plt.plot(where.get(), S_1D.get(), label=r"$\textrm{Solvent}$", color="gray")
plt.legend()
plt.yscale("log")
plt.xscale("log")
...
plt.title(
    r"$\textrm{Structure Factor at $C_s = %.1f$, $B = %.1f$}$"
    % (
        s_value,
        b_value,
    )
...
left, bottom, width, height = 0.13, 0.18, 0.25, 0.25
ax_imshow = plt.axes([left, bottom, width, height])
ax_imshow.axis("off")


# Generate sample data for the imshow plot
x_imshow = np.linspace(0, 1, 100)
y_imshow = np.linspace(0, 1, 100)
Z = np.sin(x_imshow[:, np.newaxis] * y_imshow)

# Display the imshow plot in the subplot
ax_imshow.imshow(max_species_indices_np, cmap=cmap, origin="lower")
ax_imshow.set_title(r"$\textrm{Real space}$")
...
cbar = fig.colorbar(
    im[0][0],
    cax=cax[0][0],
    ticks=unique_species_indices,
    boundaries=np.linspace(
        unique_species_indices.min() - 0.5,
        unique_species_indices.max() + 0.5,
        len(unique_species_indices) + 1,
    ),
    cmap=cmap,
)
...
struct.tight_layout()
fig.tight_layout()
fig.savefig(f"last_frame_S{s_value:.1f}_B{b_value:.1f}.pdf")

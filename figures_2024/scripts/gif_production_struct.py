...
plt.title(r"$\textrm{Structure Factor at $C_s = %.1f$, $B = %.1f$}$" % (s_value, b_value,))
...
cbar = fig.colorbar(im[0][0], cax=cax[0][0], ticks=unique_species_indices, boundaries=np.linspace(unique_species_indices.min()-0.5, unique_species_indices.max()+0.5, len(unique_species_indices)+1), cmap=cmap)

...
#    t0 = time.time()
    ...
    for poly in ps.nansatz_dict.keys():
        ...
        out = ps.nansatz_dict[poly][0](data.unsqueeze(0))
    ...
    #    print(torch.mean(out[0,1]))
        ...
        for mon in set(poly.struct.tolist()):
            amt = cp.sum(poly.h_struct[poly.struct==mon]) * ps.poly_dict[poly]
            index = ps.monomers.index(mon)
            ps.phi_all[index] += out[index] / cp.average(out[index]) * amt 

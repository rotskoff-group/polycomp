...
def load_darcy_flow_small_local(
    n_train,
    n_tests,
    batch_size,
    test_batch_sizes,
    ...
    path = '/home/emmit/Rotskoff/neuraloperator/neuralop/datasets/data'
    path = Path(__file__).resolve().parent
    return load_custom_pt(
        str(path),
        n_train=n_train,
        n_tests=n_tests,
...
def load_custom_pt(
    ...
    data = torch.load(
        Path(data_path).joinpath(f"AB_size.pt").as_posix()
    )

    x_train = data["x"][:1400].type(torch.float32).clone()
    y_train = data["y"][:1400].type(torch.float32).clone()
    del data
    ...
    x_test = data["x"][-200:].type(torch.float32).clone()
    y_test = data["y"][-200:].type(torch.float32).clone()
    del data
    ...
    for (res, n_test, test_batch_size) in zip(
        ...
        data = torch.load(Path(data_path).joinpath(f"AB_size.pt").as_posix())
        x_test = data["x"][-200:].type(torch.float32).clone()
        y_test = data["y"][-200:].type(torch.float32).clone()
        del data

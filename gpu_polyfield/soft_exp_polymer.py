...
class PolymerSystem(object):
    ...
    def get_free_energy(self, E):
        ...
        for i in range(self.normal_w.shape[0]):
            free_energy += -(1 / (2 * self.normal_evalues[i])) * cp.square(
                self.normal_w[i]
            )
        ...
        for species in self.Q_dict:
            if species in self.poly_dict:
                partition_energy -= (
                    self.poly_dict[species] * self.grid.V * cp.log(self.Q_dict[species])
                )
                ...
                salt_conc = self.salt_concs[species]
                partition_energy -= (
                    salt_conc * self.grid.V * cp.log(self.Q_dict[species])
                )
                species_partition[species] = (
                    -salt_conc * self.grid.V * cp.log(self.Q_dict[species])
                )
                # Ideal gas entropy contribution
                ig_entropy += salt_conc * self.grid.V * (cp.log(salt_conc) - 1)
            else:
        ...
        avg_conc = cp.average(
            self.reduce_phi_all(self.phi_all), axis=range(1, self.phi_all.ndim)
        )
    ...
    def get_chemical_potential(self):
        ...
        avg_red_mass = self.remove_degeneracy(
            cp.sum(self.phi_all, axis=(range(1, self.phi_all.ndim))) / self.grid.k2.size
        )
        if self.use_salts:
            self.get_salt_concs()
        for species in self.Q_dict:
            ...
            if species in self.poly_dict:
                ...
                self.chem_pot_dict[species] += alpha @ self.red_FH_mat @ avg_red_mass.T
                ...
                self.chem_pot_dict[species] += (
                    2 / 2 * (alpha @ self.red_FH_mat @ avg_red_mass.T) / self.N
                )

            elif species in self.salts:
                self.chem_pot_dict[species] -= cp.log(self.Q_dict[species])
                self.chem_pot_dict[species] += cp.log(self.salt_concs[species])
            else:
                print("Bad Species:", species)
    ...
    def get_pressure(self):
        ...
        if self.use_salts:
            self.get_salt_concs()
            for salt in self.salts:
                ideal_contribution += self.salt_concs[salt]
                Q_contribution += self.dQ_dV_dict[salt]

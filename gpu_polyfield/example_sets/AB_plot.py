...
#polymers = [AB_poly]
spec_dict = {
        A_poly : A_amt * dens_bon,
        B_poly : (1-A_amt) * dens_bon,
        }
...
relax_rates = cp.array([20 * 2 * corr * ps.grid.dV]*(ps.w_all.shape[0])) / dens_bon
print(relax_rates)
relax_temps = cp.array([0.001]*(ps.w_all.shape[0])) * dens_bon

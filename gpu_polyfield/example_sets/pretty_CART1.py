...
matplotlib.style.use("./rotskoff.mpl")
cp.random.seed(1)
...
FH_terms = {
        ...
        frozenset({A_mon, L_mon}) : tot + diff, 
        frozenset({B_mon, L_mon}) : tot + diff, 
        frozenset({S_mon, L_mon}) : tot + diff, 
        }
...
nrows=2
ncols=2
fig, axes = plt.subplots(nrows=nrows, ncols=ncols, dpi=100, figsize=(4,4))
fig.suptitle('2D Symmetric CART density profile')
...
print(im)
axes[0,0].set_title('Solvent Density')
axes[1,0].set_title('Lipid Density')
axes[1,1].set_title('Total Density')
axes[0,1].set_title('Charged Density')
#plt.show()
#exit()

free_en = []
when = 0
steps = int(10000 * 30 / 30)
for i in range(1):
    ...
    for _ in range(steps):
        ...
        hold_C += (ps.phi_all[ps.monomers.index(B_mon)].real + ps.phi_all[ps.monomers.index(A_mon)].real) / steps
    ...
    im[1][1] = axes[1,1].imshow(hold_T.get(), cmap = 'Greys')
    im[0][1] = axes[0,1].imshow(hold_C.get(), cmap = 'Reds')
...
#    cp.save('midpoint_psi', ps.psi)
    dominant_species = cp.empty(hold_T.shape, dtype=object)
    all_dens = cp.stack((hold_S - cp.amin(hold_S), hold_C - cp.amin(hold_C), hold_L - cp.amin(hold_L)))
    best = cp.argmax(all_dens, axis=0)


cvals  = [0, 1, 2]
colors = ["#258D47","#6E5AA8","#3686C0"]
colors = ["white","blue","orange"]
col_dict = dict(zip(cvals, colors))
labels = np.array(["Solvent", "Charged", "Lipid"])
...
fig2, axes2 = plt.subplots(nrows=1, ncols=1, dpi=200, figsize=(3,3))
fig2.suptitle('Highest Density Species')
im2 = axes2.imshow(best.get(), cmap=cm, norm=norm)

diff = norm_bins[1:] - norm_bins[:-1]
tickz = norm_bins[:-1] + diff / 2
print(tickz)
cb2 = fig2.colorbar(im2, format=fmt, ticks=tickz)
...
#cp.savetxt("CART1_traj/traj_%.3f" % A_amt, free_en_traj)

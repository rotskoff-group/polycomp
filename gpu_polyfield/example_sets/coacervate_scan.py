...
#    hold_L = cp.zeros_like(ps.phi_all[0].real)
    ...
    for _ in range(steps):
        ...
        net_mu.append((ps.chem_pot_dict[AB_poly], ps.chem_pot_dict[S_mon]))
        integrator_1.ETD()
        when += 1
...
full_traj = cp.concatenate((free_en_traj, net_mu_traj), axis=1)
cp.savetxt("CA_traj/traj_%.10f" % CA_amt, full_traj)
print("AS_traj/traj_%.10f" % CA_amt)

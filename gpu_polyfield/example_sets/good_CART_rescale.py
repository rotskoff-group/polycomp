...
tot = 200 / dens_corr
tot = 200
#lip_pen = 2.4 / dens_corr
lip_pen = 1.15 / dens_corr
#lip_pen = 3.1 / dens_corr
FH_terms = {
        frozenset({A_mon}) : tot, 
        frozenset({B_mon}) : tot, 
        frozenset({S_mon}) : tot, 
...
relax_rates = cp.array([4 * 2 * corr * ps.grid.dV]*(ps.w_all.shape[0])) / dens_corr
print(relax_rates)
#relax_rates[0] *= 2
relax_temps = cp.array([0.001]*(ps.w_all.shape[0])) * dens_corr
relax_temps *= ps.gamma.real
psi_rate = 2.0 * ps.grid.dV / dens_corr 
print(psi_rate)
#psi_rate = 0.006096631 * N**2
#psi_rate = 20 * N / 500
psi_temp = 1 * dens_corr * 1
E = 350 
E = 700 
E = 1000
E = 4100

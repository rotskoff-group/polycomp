...
nanp1, A_brush_dens = draw_circle(cp.array([9,9]), 2, grid)
nanp2, B_brush_dens = draw_circle(cp.array([15,15]), 2, grid)
#hold[hold < cp.amax(hold)] = 0 
...
B_brush = p.Brush('brush', B_brush_dens)
...
FH_terms = {
        ...
        frozenset({A_mon, N_mon}) : repul + extra, 
        frozenset({B_mon, C_mon}) : repul + extra, 
        frozenset({B_mon, S_mon}) : repul + extra, 
        frozenset({B_mon, N_mon}) : repul + extra, 
        frozenset({C_mon, S_mon}) : repul + extra, 
        frozenset({C_mon, N_mon}) : repul + extra, 
        frozenset({N_mon, S_mon}) : repul + extra, 
        }
...
A_poly = p.Polymer("A", N, [(C_mon, .875 + 1.875), (A_mon, .125)], fastener=A_brush)
B_poly = p.Polymer("B", N2, [(C_mon, .875 - .5), (B_mon, .125)], fastener=B_brush)
#A_poly = p.Polymer("A", N, [(C_mon, .5), (A_mon, .3)])
#B_poly = p.Polymer("B", N2, [(C_mon, .5), (B_mon, .3)])
...
nanp = p.Nanoparticle("N", N_mon, (nanp1 + nanp2) / (cp.average(nanp1[nanp1 > 0]) +cp.average( nanp2[nanp2 > 0])) * 2 * 10)
nanp_list = [nanp]
smear = 0.1632993161855452 / 4
print(grid.dV**(1/3) / smear)

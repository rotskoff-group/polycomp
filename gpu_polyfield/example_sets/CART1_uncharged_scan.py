...
FH_terms = {
        ...
        frozenset({A_mon, L_mon}) : 1.5 + diff, 
        frozenset({B_mon, L_mon}) : 1.5 + diff, 
        frozenset({S_mon, L_mon}) : 1.5 + diff, 
        }
...
ps = p.PolymerSystem(monomers, polymers, spec_dict, FH_terms,
        grid, smear, salt_conc=0.020, integration_width = 1/10)
...
relax_rates = cp.array([8 * 2 * corr * ps.grid.dV]*(ps.w_all.shape[0])) 
print(relax_rates)
#relax_rates[0] *= 2
...
#cp.savetxt("CART1_uncharged_traj/traj_%.3f" % A_amt, free_en_traj)

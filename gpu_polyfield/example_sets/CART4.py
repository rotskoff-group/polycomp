...
from se_plot_helpers import clip
import cupy as cp
import math
...
FH_terms = {
        ...
        frozenset({S_mon}) : 0.5, 
        frozenset({L_mon}) : 0.5, 
        frozenset({A_mon, B_mon}) : 0.5, 
        frozenset({A_mon, S_mon}) : 0.5, 
        frozenset({B_mon, S_mon}) : 0.5, 
        frozenset({A_mon, L_mon}) : 0.5 + 3, 
        frozenset({B_mon, L_mon}) : 0.5 + 3, 
        frozenset({S_mon, L_mon}) : 0.5 + 3, 
        }
...
grid_spec = (230,230)
grid_spec = (23 * 26 // 1,23 * 26 // 1)
#grid_spec = (420,420)
box_length = (20,20)
box_length = (20 * 2,20 * 2)
...
relax_rates = cp.array([2 * 1 * corr * ps.grid.dV]*(ps.w_all.shape[0])) 
print(relax_rates)
#relax_rates[0] *= 2
relax_temps = cp.array([0.001]*(ps.w_all.shape[0])) 
relax_temps *= ps.gamma.real
psi_rate = 10 * ps.grid.dV
print(psi_rate)
...
fig.suptitle('CART-Homopolymer 1:1 Lipid:Charge, clipped outliers')
...
#ps.update_normal_from_density()
#ps.normal_w[-1].real = ps.randomize_array(ps.w_all[0], 30)
#ps.update_density_from_normal()
...
for i in range(30):
    ...
    for _ in range(steps):
        integrator_1.ETD()
        hold_S += clip(ps.phi_all[ps.monomers.index(S_mon)].real, clip_min, clip_max) / steps
        hold_A += clip(ps.phi_all[ps.monomers.index(A_mon)].real, clip_min, clip_max) / steps
        hold_B += clip(ps.phi_all[ps.monomers.index(B_mon)].real, clip_min, clip_max) / steps
        hold_L += clip(ps.phi_all[ps.monomers.index(L_mon)].real, clip_min, clip_max) / steps
        hold_T += clip(cp.sum(ps.phi_all, axis=0).real, clip_min, clip_max) / steps
        hold_C += clip(ps.get_total_charge().real, clip_min, clip_max) / steps
    charge_max = cp.amax(cp.abs(hold_C))
    im[0][0] = axes[0,0].imshow(hold_S.get(), cmap = 'Greens', vmin = 0)
    im[1][0] = axes[1,0].imshow(hold_L.get(), cmap = 'Purples', vmin = 0)
    im[2][0] = axes[2,0].imshow(hold_T.get(), cmap = 'Greys', vmin = 0)
    im[0][1] = axes[0,1].imshow(hold_A.get(), cmap = 'Blues', vmin = 0)
    im[1][1] = axes[1,1].imshow(hold_B.get(), cmap = 'Reds', vmin = 0)

...
#polymers = [AB_poly]
spec_dict = {
        AB_poly : 0.5 * dens_corr,
        S_mon : 0.0000001 * N * dens_corr
        }
#spec_dict = {
#        A_poly : .0001 * 2,
#        B_poly : .0001 * 2,
...
ps = p.PolymerSystem(monomers, polymers, spec_dict, FH_terms,
        grid, smear, salt_conc=0.0 * N, integration_width = 1/10)
...
relax_rates = cp.array([2 * 2 * corr * ps.grid.dV]*(ps.w_all.shape[0])) / dens_corr 
print(relax_rates)
#relax_rates[0] *= 2
relax_temps = cp.array([0.001]*(ps.w_all.shape[0])) * dens_corr
relax_temps *= ps.gamma.real
psi_rate = 20 * ps.grid.dV * 2 / dens_corr
print(psi_rate)
#psi_rate = 0.006096631 * N**2
#psi_rate = 20 * N / 500
psi_temp = 1 * dens_corr
E = 350 
E = 1700 
E = 4100
E = 1000 / dens_corr

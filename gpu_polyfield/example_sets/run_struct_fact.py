...
import se_struct_fact as s_fact
...
ps = p.PolymerSystem(monomers, polymers, spec_dict, FH_terms,
        box_length, grid_spec, smear, salt_conc=0.020, integration_width = 1/7)
...
ps.w_all = cp.load('midpoint.npy')
ps.update_normal_from_density()
ps.psi = cp.load('midpoint_psi.npy')
ps.get_densities()
im = []
div = []
cax = [] 
...
dists, struct1d, struct2d = s_fact.get_structure_factor(ps.grid, ps.phi_all[ps.monomers.index(L_mon)].real)
its = 400
for i in range(its):
    print(i)
    integrator_1.ETD()
    hold_dist, hold_str, hold_str2 = s_fact.get_structure_factor(ps.grid, ps.phi_all[ps.monomers.index(L_mon)].real)
    struct1d += hold_str
struct1d /= (its + 1)
...
im2 = axes2.imshow(struct2d.get())
fig3, axes3 = plt.subplots()
axes3.plot(dists.get(), struct1d.get())
